...
class SharedPdfActivty : AppCompatActivity() {
    ...
    fun Shared(view: View){
        ...
        if (selectedItem !=null){
            pdfReference.putFile(selectedItem!!).addOnSuccessListener {
                taskSnapshot ->
                val uploadPdfReference = FirebaseStorage.getInstance().reference.child("PDFCollections").child(pdfNames)
                uploadPdfReference.downloadUrl.addOnSuccessListener {
                    uri ->
                    val downloadUrl = uri.toString()

                    val currentuserEmail = auth.currentUser!!.email.toString()
                    val pdfName = pdfName.text.toString()  //kullanıcı yorumu alma işlemi
                    ...
                    database.collection("PDFCollections").add(pdfHashMap).addOnCompleteListener{
                        task ->
